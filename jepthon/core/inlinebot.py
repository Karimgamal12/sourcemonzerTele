...
ROZLOGO = "https://telegra.ph/file/6ddb733167cbf0ff07102.jpg"
...
def main_menu():
    text = f"**- [سـورس زد إي](https://t.me/UI_9U)\
        \n المسـاعد\
        \n\
        \n◽ زد إي لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس زد إي 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس زد إي 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس زد إي 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/UI_9U"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/UI_OS",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت زد إي 🧸♥](https://t.me/UI_9U)**\
            \n\
            \n❤ زد إي هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/UI_OS)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
