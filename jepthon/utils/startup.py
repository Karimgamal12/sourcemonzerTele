...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/6e5216b3ce048999e1722.jpg",
                caption="**᯽︙ بــوت افاتار يـعمل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس افتار", "https://t.me/sourceav"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت افاتار تمت بنجاح**"
            )
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @UI_9U<,"
        photobt = await jepiq.upload_file(file="jepthon-master/JepIQ/razan/resources/start/Av.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات افاتار, ", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jepiq.upload_file(file="jepthon-master/JepIQ/razan/resources/start/Av2.JPEG")
        _, groupid = await create_supergroup(
